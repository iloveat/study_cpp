...
void interp1(const double *x, const double *y, int x_length, const double *xi, int xi_length, double *yi)
{
    ...
    h = (double *)malloc(sizeof(double)*(x_length - 1));
    p = (double *)malloc(sizeof(double)*(xi_length));
    s = (double *)malloc(sizeof(double)*(xi_length));
    k = (int *)malloc(sizeof(int)*(xi_length));
...
void decimate(const double *x, int x_length, int r, double *y)
{
    ...
    double *tmp1 = NULL;
    double *tmp2 = NULL;
    tmp1 = (double *)malloc(sizeof(double)*(x_length + kNFact * 2));
    tmp2 = (double *)malloc(sizeof(double)*(x_length + kNFact * 2));
...
{
	double *xi_fraction = NULL;
	double *delta_y = NULL;
	int *xi_base = NULL;

	xi_fraction = (double *)malloc(sizeof(double)*xi_length);
	delta_y = (double *)malloc(sizeof(double)*xi_length);
	xi_base = (int *)malloc(sizeof(int)*xi_length);
...
void fast_fftfilt(const double *x,
                  int x_length,
                  const double *h,
                  int h_length,
                  int fft_size,
                  ...
                  double *y)
{
    fft_complex *x_spectrum = NULL;
    x_spectrum = (fft_complex *)malloc(sizeof(fft_complex)*fft_size);
    ...
    for (int i = 0; i <= fft_size / 2; ++i)
    {
        inverse_real_fft->spectrum[i][0] = x_spectrum[i][0] * forward_real_fft->spectrum[i][0] - x_spectrum[i][1] * forward_real_fft->spectrum[i][1];
        inverse_real_fft->spectrum[i][1] = x_spectrum[i][0] * forward_real_fft->spectrum[i][1] + x_spectrum[i][1] * forward_real_fft->spectrum[i][0];
    }
