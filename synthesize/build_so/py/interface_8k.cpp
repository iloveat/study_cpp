							 ...
							 int &pcm_length)
{
	...
	int num_column = num_spectrum+1+1;  //note: when in 8k, bap=1 (actually bap=0, we use bap=1 for coding simplicity)
	if(num_column != cols) {
...
*/
	...
	int y_length = (int)((num_frame-num_skip*2)*5.0/1000*8000);  // 8000
...
static PyObject* 
ExVocoder8K_synthesizeMemoMemoPCM(PyObject *self, PyObject *args)
{
	int offline;
	int num_spectrum;
	int num_skip;
...
ExVocoder8KMethods[] =
{
	{ "synthesize_memo_memo_pcm", ExVocoder8K_synthesizeMemoMemoPCM, METH_VARARGS },
	{ NULL, NULL },  
};

void initExVocoder8K()
{
	Py_InitModule("ExVocoder8K", ExVocoder8KMethods);
}
