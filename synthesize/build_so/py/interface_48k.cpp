							 ...
							 int &pcm_length)
{
	...
	int num_column = num_spectrum+1+5;  //note: when in 48k, bap=5
	if(num_column != cols) {
...
*/
	...
	int y_length = (int)((num_frame-num_skip*2)*5.0/1000*48000);  // 48000
...
static PyObject* 
ExVocoder48K_synthesizeMemoMemoPCM(PyObject *self, PyObject *args)
{
	int offline;
	int num_spectrum;
	int num_skip;
...
ExVocoder48KMethods[] =
{
	{ "synthesize_memo_memo_pcm", ExVocoder48K_synthesizeMemoMemoPCM, METH_VARARGS },
	{ NULL, NULL },  
};

void initExVocoder48K()
{
	Py_InitModule("ExVocoder48K", ExVocoder48KMethods);
}
