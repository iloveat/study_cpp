...
** File:    lpc.c
**
** √Ë ˆ: Functions that implement linear predictive coding 
**      (LPC) operations.  
**
** Functions:
**
...
** Function:        Comp_Lpc()
**
** √Ë ˆ:     Computes the tenth-order LPC filters for an
...
** Arguments:
**
**  Q15 emInt16 *UnqLpc      Empty Buffer
**  Q15 emInt16 PrevDat[]    Previous 2 subframes of samples (120 words)
**  Q15 emInt16 DataBuff[]   Current frame of samples (240 words)
**
** Outputs:

**
**  Q15 emInt16 UnqLpc[]     LPC coefficients for entire frame (40 words)
**
** ∑µªÿ value:    None
**
*/
void  Comp_Lpc( NEED_THIS Q15 emInt16 *UnqLpc, Q15 emInt16 *PrevDat, Q15 emInt16 *DataBuff )
{
    emInt32    i,j,k ;

    Q15 emInt16   Dpnt[Frame+LpcFrame-SubFrLen] ;
    Q15 emInt16   Vect[LpcFrame] ;
    Q15 emInt16   Acf_sf[LpcOrderP1*SubFrames];
    Q15 emInt16   ShAcf_sf[SubFrames];
    Q15 emInt16   Exp   ;
    Q15 emInt16   *curAcf;
    Q15 emInt16   Pk2;
     ...
     */
    for ( i = 0 ; i < LpcFrame-SubFrLen ; i ++ )///180-60
		Dpnt[i] = PrevDat[i] ;
    for ( i = 0 ; i < Frame ; i ++ )
    ...
    for ( k = 0 ; k < SubFrames ; k ++ ) {
        ...
        */
        ...
        /* Normalize */
        ShAcf_sf[k] = Vec_Norm( Vect, (Q15 emInt16) LpcFrame ) ;

        /* Apply the Hamming window */
        for ( i = 0 ; i < LpcFrame ; i ++ )
            Vect[i] = mult_r_1(Vect[i], HammingWindowTable[i]) ;
        ...
        */
        ...
        Acc0 = L_shr( Acc1, (Q15 emInt16) RidgeFact ) ;
        Acc1 = L_add( Acc1, Acc0 ) ;

        /* Normalize the energy */
        Exp = norm_l( Acc1 ) ;
        Acc1 = L_shl_1( Acc1, Exp ) ;
    ...
    Update_Acf(USE_THIS Acf_sf, ShAcf_sf);
}
...
** Function:        Durbin()
**
** √Ë ˆ:     Implements the Levinson-Durbin algorithm for a
...
** Arguments:       
**
**  Q15 emInt16 *Lpc Empty buffer
**  Q15 emInt16 Corr[]   First- through tenth-order autocorrelations (10 words)
**  Q15 emInt16 Err  Zeroth-order autocorrelation, or energy
**
** Outputs:     
**
**  Q15 emInt16 Lpc[]    LPC coefficients (10 words)
**
** ∑µªÿ value:    The error
**
*/
Q15 emInt16  Durbin( Q15 emInt16 *Lpc, Q15 emInt16 *Corr, Q15 emInt16 Err, Q15 emInt16 *Pk2 )
{
    emInt32    i,j   ;

    Q15 emInt16   Temp[LpcOrder] ;
    Q15 emInt16   Pk ;
 ...
 */
        ...
        Acc0 = L_shr_1( Acc0, (Q15 emInt16) 2 ) ;
        for ( j = 0 ; j < i ; j ++ )
...
** Function:        Wght_Lpc()
**
** √Ë ˆ:     Computes the formant perceptual weighting
...
** Arguments:       
**
**  Q15 emInt16 *PerLpc      Empty Buffer
**  Q15 emInt16 UnqLpc[]     Unquantized LPC coefficients (40 words)
**
** Outputs:     

**
**  Q15 emInt16 PerLpc[]     Perceptual weighting filter coefficients
**              (80 words)
**
** ∑µªÿ value:    None
**
*/
void  Wght_Lpc( Q15 emInt16 *PerLpc, Q15 emInt16 *UnqLpc )
{
    emInt32    i,j   ;
  ...
  */
        for ( j = 0 ; j < LpcOrder ; j ++ )
            PerLpc[j] = mult_r_1( UnqLpc[j], PerFiltZeroTable[j] ) ;
        PerLpc += LpcOrder ;
 ...
 */
        for ( j = 0 ; j < LpcOrder ; j ++ )
            PerLpc[j] = mult_r_1( UnqLpc[j], PerFiltPoleTable[j] ) ;
        PerLpc += LpcOrder ;
        UnqLpc += LpcOrder ;
    }
}
...
** Function:        Error_Wght()
**
** √Ë ˆ:     Implements the formant perceptual weighting
...
** Arguments:
**
**  Q15 emInt16 Dpnt[]       Highpass filtered speech x[n] (240 words)
**  Q15 emInt16 PerLpc[]     Filter coefficients (80 words)
**
** Inputs:
**
...
** Outputs:
**
**  Q15 emInt16 Dpnt[]       Weighted speech f[n] (240 words)
**
** ∑µªÿ value:    None
**
*/
void  Error_Wght( NEED_THIS Q15 emInt16 *Dpnt, Q15 emInt16 *PerLpc )
{
    emInt32    i,j,k ;
 ...
 */
            /* Filter */
            Acc0 = L_mult( *Dpnt, (Q15 emInt16) 0x2000 ) ;
            for ( j = 0 ; j < LpcOrder ; j ++ )
...
** Function:        Comp_Ir()
**
** √Ë ˆ:     Computes the combined impulse response of the
...
** Arguments:
**
**  Q15 emInt16 *ImpResp     Empty Buffer
**  Q15 emInt16 QntLpc[]     Quantized LPC coefficients (10 words)
**  Q15 emInt16 PerLpc[]     Perceptual filter coefficients (20 words)
**  PWDEF Pw        Harmonic noise shaping filter parameters
**
** Outputs:
**
**  Q15 emInt16 ImpResp[]    Combined impulse response (60 words)
**
** ∑µªÿ value:    None
**
*/
void  Comp_Ir( Q15 emInt16 *ImpResp, Q15 emInt16 *QntLpc, Q15 emInt16 *PerLpc, PWDEF Pw )
{
    emInt32    i,j   ;

    Q15 emInt16   FirDl[LpcOrder] ;
    Q15 emInt16   IirDl[LpcOrder] ;
    Q15 emInt16   Temp[PitchMax+SubFrLen] ;
...
** Function:        Sub_Ring()
**
** √Ë ˆ:     Computes the zero-input response of the
...
** Arguments:       
**
**  Q15 emInt16 Dpnt[]       Harmonic noise weighted vector w[n] (60 words)
**  Q15 emInt16 QntLpc[]     Quantized LPC coefficients (10 words)
**  Q15 emInt16 PerLpc[]     Perceptual filter coefficients (20 words)
**  Q15 emInt16 PrevErr[]    Harmonic noise shaping filter memory (145 words)
**  PWDEF Pw        Harmonic noise shaping filter parameters
**  
** Inputs:
**
...
** Outputs:     
**
**  Q15 emInt16 Dpnt[]       Target vector t[n] (60 words)
**
** ∑µªÿ value:    None
**
*/
void  Sub_Ring( NEED_THIS Q15 emInt16 *Dpnt, Q15 emInt16 *QntLpc, Q15 emInt16 *PerLpc, Q15 emInt16
*PrevErr, PWDEF Pw )
{
    emInt32    i,j   ;
    Q31 emInt32   Acc0,Acc1 ;
  ...
  */
        ...
        Acc0 = L_mac( Acc0, Pw.Gain, Temp[PitchMax-(emInt32 )Pw.Indx+i] ) ;
        Dpnt[i] = round ( Acc0 ) ;
    }
}
...
** Function:        Upd_Ring()
**
** √Ë ˆ:     Updates the memory of the combined formant
...
** Arguments:       
**
**  Q15 emInt16 Dpnt[]       Decoded excitation for the current subframe e[n] 
**               (60 words)
**  Q15 emInt16 QntLpc[]     Quantized LPC coefficients (10 words)
**  Q15 emInt16 PerLpc[]     Perceptual filter coefficients (20 words)
**  Q15 emInt16 PrevErr[]    Harmonic noise shaping filter memory (145 words)
...
** Outputs:     
**
**  Q15 emInt16 PrevErr[]    Updated harmonic noise shaping filter memory 
...
**
*/
void  Upd_Ring( NEED_THIS Q15 emInt16 *Dpnt, Q15 emInt16 *QntLpc, Q15 emInt16 *PerLpc, Q15 emInt16 *PrevErr )
{
    emInt32    i,j   ;
  ...
  */
        ...
        Acc0 = L_shr_1( Acc0, (Q15 emInt16) 3 ) ;
  ...
  */
        ...
        Dpnt[i] = shl_1( round( Acc1 ), (Q15 emInt16) 1 ) ;
...
** Function:        Spf()
**
** √Ë ˆ:     Implements the formant postfilter for a
...
** Arguments:
**
**  Q15 emInt16 Tv[]     Synthesized speech vector sy[n] (60 words)
...
** Outputs:
**
**  Q15 emInt16 Tv[]     Postfiltered speech vector pf[n] (60 words)
...
**
** ∑µªÿ value: Input vector energy
**
*/

Q31 emInt32  Spf( NEED_THIS Q15 emInt16 *Tv, Q15 emInt16 *Lpc )
{
    emInt32    i,j   ;
    ...
    Q15 emInt16   FirCoef[LpcOrder] ;
    Q15 emInt16   IirCoef[LpcOrder] ;

    Q15 emInt16   TmpVect[SubFrLen] ;
  ...
  */
    for ( i = 0 ; i < LpcOrder ; i ++ ) {
        FirCoef[i] = mult_r_1( Lpc[i], PostFiltZeroTable[i] ) ;
        IirCoef[i] = mult_r_1( Lpc[i], PostFiltPoleTable[i] ) ;
    }
  ...
  */
    for ( i = 0 ; i < SubFrLen ; i ++ )
        TmpVect[i] = Tv[i] ;
    Exp = Vec_Norm( TmpVect, (Q15 emInt16) SubFrLen ) ;
  ...
  */
    Sen = L_shr_1( Acc1, (Q15 emInt16)(2*Exp + 4) ) ;
  ...
  */
    ...
    Acc0 = L_msu( Acc0, DecStat.Park, (Q15 emInt16) 0x2000 ) ;
    Acc0 = L_mac( Acc0, Tmp, (Q15 emInt16) 0x2000 ) ;
    DecStat.Park = round( Acc0 ) ;

    Tmp  = mult_1( DecStat.Park, PreCoef ) ;
    Tmp &= (Q15 emInt16) 0xfffc ;
