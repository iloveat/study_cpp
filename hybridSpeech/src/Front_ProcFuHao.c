...
/*
 *modified by zhiba
 * add du (degree)0xb6c8 to system
 * example: from nWord== (emInt16)0xa1e6  to nWord== (emInt16)0xa1e6 ||nWord ==(emInt16)0xb6c8
 *
 */
		...
		if( nWord == (emInt16)0xa3ad || nWord == (emInt16)0xa1aa )          //特殊符号“-”   ------6月3日写 或着”—“
		{
			...
			if( nDataType == (emInt16)DATATYPE_SHUZI)//前一个字符类型为数字
			{
				...
				if(nDataType == (emInt16)DATATYPE_SHUZI)//特殊符号”-“后一个字符类型也为数字
                {
                    ...
                    nWord1=GetWord( strDataBuf, nCurIndexOfBuf + 2+2*nSumOfShuZi);
                    ...
                    */
                    //modified by zhiba add jian
                    else if(nWord1==(emInt16)0xa3ad||nWord1==(emInt16)0xb5c8||nWord1==(emInt16)0xcac7||nWord1==(emInt16)0xa3bd)
                    {

                        PutIntoWord( g_hTTS->m_ShuZiBuffer,0, 0xbcf5 ); //jian sub
                        PutIntoWord( g_hTTS->m_ShuZiBuffer, 2, END_WORD_OF_BUFFER );
			...
			if( nDataType == (emInt16)DATATYPE_YINGWEN )//前一个字符类型为英文
			{
				...
				{
					nSumOfShuZi = GetStrSum( strDataBuf, nCurIndexOfBuf + 2, (emInt16)DATATYPE_SHUZI );
                    nWord = GetWord( strDataBuf, nCurIndexOfBuf + 2 * ( nSumOfShuZi + 1) ) ;
                    if(( nWord== (emInt16)0xa1e6 ||nWord ==(emInt16)0xb6c8)) //数字后面恰好是℃符号识别成零下
					{
						//处理成零下摄氏度并播放（s-12℃）
					...
					if((nWord = GetWord( strDataBuf, nCurIndexOfBuf + 2 * ( nSumOfShuZi + 1))) == (emInt16)0xa3ae )//数字后面有点（带小数的数字）
					{
                        ...
                        nWord = GetWord ( strDataBuf, nCurIndexOfBuf + 2 * ( nSumOfShuZi + nSumOfReadHaoMa + 2 ));
                        if( nSumOfReadHaoMa > 0 && ((nWord  == (emInt16)0xa1e6||nWord ==(emInt16)0xb6c8) ))//小数点后面的数字的后面是℃同时空格的前面不是℃
						{
			...
			if( nDataType == (emInt16)DATATYPE_FUHAO)//前一个字符类型为符号
			{
				if( (nWord = GetWord( strDataBuf, nCurIndexOfBuf - 2)) == (emInt16)0xa1e6 )//特殊符号'-'前一个符号恰好是℃
				{
					...
					if( nDataType == (emInt16)DATATYPE_SHUZI )//后一个字符类型为数字
					{
						nSumOfShuZi = GetStrSum( strDataBuf, nCurIndexOfBuf + 2, (emInt16)DATATYPE_SHUZI );
                        nWord = GetWord( strDataBuf, nCurIndexOfBuf + 2 * ( nSumOfShuZi + 1) ) ;
                        if(( nWord == (emInt16)0xa1e6 ||nWord ==(emInt16)0xb6c8) )//数字后面恰好是℃符号
						{
							//读成至（12℃-12℃）
							PutIntoWord( g_hTTS->m_ShuZiBuffer,0, 0xd6c1 ); //至
							PutIntoWord( g_hTTS->m_ShuZiBuffer, 2, END_WORD_OF_BUFFER );
						...
						if((nWord = GetWord( strDataBuf, nCurIndexOfBuf + 2 * ( nSumOfShuZi + 1))) == (emInt16)0xa3ae )//数字后面有点（带小数的数字）
						{
                            nSumOfReadHaoMa = GetStrSum( strDataBuf, nCurIndexOfBuf + nSumOfShuZi * 2 + 4,(emInt16)DATATYPE_SHUZI );
                                nWord = GetWord ( strDataBuf, nCurIndexOfBuf + 2 * ( nSumOfShuZi + nSumOfReadHaoMa + 2 ));
                            if( nSumOfReadHaoMa > 0 && ((nWord== (emInt16)0xa1e6)||(nWord== (emInt16)0xa1e6)) )//小数点后面的数字的后面是℃
							{
								//读成至（12℃-12.9℃）
								PutIntoWord( g_hTTS->m_ShuZiBuffer,0, 0xd6c1 ); //至
								PutIntoWord( g_hTTS->m_ShuZiBuffer, 2, END_WORD_OF_BUFFER );
				...
				if( nWord != (emInt16)0xa1eb && nWord != (emInt16)0xa987 && nWord != (emInt16)0xa3a5  )
				{
					...
					if( nDataType == (emInt16)DATATYPE_SHUZI )//后一个字符类型为数字
					{
                        ...
                        if(( (nWord = GetWord( strDataBuf, nCurIndexOfBuf + 2 * ( nSumOfShuZi + 1) ) ) == (emInt16)0xa1e6 ||
                             (nWord = GetWord( strDataBuf, nCurIndexOfBuf + 2 * ( nSumOfShuZi + 1) ) ) == (emInt16)0xb6c8 )
                            && (( nWord = GetWord( strDataBuf, nCurIndexOfBuf - 2 ) ) != (emInt16)0xa1e6 ||
                                 (nWord = GetWord( strDataBuf, nCurIndexOfBuf - 2 ) ) != (emInt16)0xb6c8 )) //数字后面恰好是℃符号识别成零下
						{
							//处理成零下摄氏度并播放（好-12℃）
							PutIntoWord( g_hTTS->m_ShuZiBuffer ,  0 , (emInt16)0xc1e3 ); //零		
							PutIntoWord( g_hTTS->m_ShuZiBuffer ,   2, (emInt16)0xcfc2 ); //下
						...
						if((nWord = GetWord( strDataBuf, nCurIndexOfBuf + 2 * ( nSumOfShuZi + 1))) == (emInt16)0xa3ae )//数字后面有点（带小数的数字）
						{
                            ...
                            if( nSumOfReadHaoMa > 0 && ((nWord = GetWord ( strDataBuf, nCurIndexOfBuf + 2 * ( nSumOfShuZi + nSumOfReadHaoMa + 2 ))) == (emInt16)0xa1e6
                                                        ||(nWord = GetWord ( strDataBuf, nCurIndexOfBuf + 2 * ( nSumOfShuZi + nSumOfReadHaoMa + 2 ))) == (emInt16)0xb6c8)
                                &&( ( nWord = GetWord( strDataBuf, nCurIndexOfBuf - 2 ) ) != (emInt16)0xa1e6
                                 ||(nWord = GetWord( strDataBuf, nCurIndexOfBuf - 2 ) ) != (emInt16)0xb6c8))//小数点后面的数字的后面是℃
							{
			...
			if( nDataType == (emInt16)DATATYPE_UNKNOWN)//前一个字符类型为未知 诸如句子的5个结束标点
			{
				...
				if( nDataType == (emInt16)DATATYPE_SHUZI )//后一个字符类型为数字
				{
					nSumOfShuZi = GetStrSum( strDataBuf, nCurIndexOfBuf + 2, (emInt16)DATATYPE_SHUZI );
                     nWord = GetWord( strDataBuf, nCurIndexOfBuf + 2 * ( nSumOfShuZi + 1) );
                    if(( (nWord = GetWord( strDataBuf, nCurIndexOfBuf + 2 * ( nSumOfShuZi + 1) ))== (emInt16)0xa1e6
                         ||(nWord = GetWord( strDataBuf, nCurIndexOfBuf + 2 * ( nSumOfShuZi + 1) ))==0xb6c8)
                        && (( nWord = GetWord( strDataBuf, nCurIndexOfBuf - 2 ) ) != (emInt16)0xa1e6
                            ||(nWord = GetWord( strDataBuf, nCurIndexOfBuf - 2 ) ) != (emInt16)0xb6c8) ) //数字后面恰好是℃符号识别成零下
					{
						//处理成零下摄氏度并播放（好-12℃）
                    ...
                    if(  nWord == (emInt16)0xb4bd //dao
                         ||nWord == (emInt16)0xb5bd  //zhi
                        || nWord == (emInt16)0xa1ab  //符号：～
						...
						|| (nWord == (emInt16)0xa3a0 &&  (nNextWord == (emInt16)0xa1ab || nNextWord == (emInt16)0xa3fe) ))//符号：空格～  或 空格~
					{
						for( j=2; j<16;j=j+2)
						{
                            ...
                            if( nWord == (emInt16)0xa1e6||nWord==(emInt16)0xb6c8 ) //符号：℃
								break;
						}
			...
			if( nDataType == (emInt16)DATATYPE_SHUZI )//后一个字符类型为数字
			{
				...
				if((nWord = GetWord( strDataBuf, nCurIndexOfBuf + 2 * ( nSumOfShuZi + 1))) == (emInt16)0xa3ae )//数字后面有点（带小数的数字）
				{
                    nSumOfReadHaoMa = GetStrSum( strDataBuf, nCurIndexOfBuf + nSumOfShuZi * 2 + 4,(emInt16)DATATYPE_SHUZI );
                    nWord = GetWord ( strDataBuf, nCurIndexOfBuf + 2 * ( nSumOfShuZi + nSumOfReadHaoMa + 2 ));
                    if( nSumOfReadHaoMa > 0 && ( nWord== (emInt16)0xa1e6||nWord == (emInt16)0xb6c8) )//小数点后面的数字的后面是℃
					{
						//处理成零下摄氏度并播放（诸如好-12.8℃）
						PutIntoWord( g_hTTS->m_ShuZiBuffer ,  0 , (emInt16)0xc1e3 ); //零		
						PutIntoWord( g_hTTS->m_ShuZiBuffer ,   2, (emInt16)0xcfc2 ); //下
