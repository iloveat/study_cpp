...
int pWinSize[]={24,29,35,41,50,60,72,86,103,124,149,178,214,257,308,370,444,532,639,767,920,1104,1325,1590,1908,2290,2747,3297,3956,4000};
...
GAB::GAB(){
  ...
  {
    for(unsigned int j = i+1; j < numPixels; j ++)
    {
      lpoints.push_back(i);
      rpoints.push_back(j);
    }
...
bool GAB::LoadFrontalModel(string path){
    int pWinSize2[]={20,24,29,35,41,50,60,72,86,103,124,149,178,214,257,308,370,444,532,639,767,920,1104,1325,1590,1908,2290,2747,3297,3956};
	for(int p=0;p<30;p++)
    pWinSize[p]=pWinSize2[p];
	numScales=30;
	FILE* file;
	if((file = fopen(path.c_str(), "rb"))==NULL)
		return false;
	...
	int *_pixel1 = new int[30*numBranchNodes];
	int *_pixel2 = new int[30*numBranchNodes];

	int *_leftChild = new int[numBranchNodes];
	int *_rightChild = new int[numBranchNodes];
	unsigned char* _cutpoint = new unsigned char[2*numBranchNodes];
	fread(_pixel1,sizeof(int),numBranchNodes*30,file);
	fread(_pixel2,sizeof(int),numBranchNodes*30,file);
	...
	for(int i = 0;i<numBranchNodes;i++){
        leftChilds.push_back(_leftChild[i]);
        rightChilds.push_back(_rightChild[i]);
        cutpoints.push_back(_cutpoint[i]);
        cutpoints.push_back(_cutpoint[i+numBranchNodes]);
	}

	for(int i=0;i<30;i++){
		for(int j=i*numBranchNodes;j<(i+1)*numBranchNodes;j++){
            points1[i].push_back(_pixel1[j]);
            points2[i].push_back(_pixel2[j]);
        }
	}
...
//Detect face....
vector<int> GAB::DetectFace(Mat img,vector<Rect>& rects,vector<float>& scores, int minFace){
  ...
  for(unsigned int i = 0;i<picked.size();i++){
    ...
    int y0 = max(rects[idx].y - floor(3.0 * delta),1);
    int y1 = min(rects[idx].y + Srect[idx]*1.25,imgHeight);
    int x0 = max(rects[idx].x + floor(0.25 * delta),1);
    int x1 = min(rects[idx].x + Srect[idx]*1.1 - floor(0.25 * delta),imgWidth);

//    int y0 = max(rects{i}(j).row - floor(2.5 * delta), 1);
//    int y1 = min(rects{i}(j).row + rects{i}(j).size - 1 + floor(2.5 * delta), imgHeight);
//    int x0 = max(rects{i}(j).col - delta, 1);
//    int x1 = min(rects{i}(j).col + rects{i}(j).size - 1 + delta, imgWidth);
